...
describe('GET ALL /api/task', function() {
    it('should GET all the tasks', function(done) {
        chai.request(server).get('/api/tasks').end((err, response) => {
    ...
    it('should NOT GET all the tasks', function(done){
        chai.request(server).get('/api/task').end((err, response) => {
...
describe('GET task by ID', () => {
    it('should GET a task by ID', (done) => {
        const taskId = 1
        chai.request(server).get('/api/tasks/' + taskId).end((err, response) => {
            response.should.have.status(200)
...
describe('POST /api/tasks', () => {
    it('should POST a new task', (done) => {
        ...
        chai.request(server).post('/api/tasks').send(task).end((err, response) => {
...
describe('PUT /api/tasks/:id', () => {
    it('should PUT an existing task', (done) => {
        ...
        chai.request(server).put('/api/tasks/' +taskId).send(task).end((err, response) => {
            response.should.have.status(200)
            response.body.should.be.a('object')
            response.body.should.have.property('id').eq(1)
...
describe('PATCH /api/tasks/:id', () => {
    it('should PATCH an existing task', (done) => {
        ...
        chai.request(server).patch('/api/tasks/' + taskId).send(task).end((err, response) => {
            response.should.have.status(200)
            response.body.should.be.a('object')
            response.body.should.have.property('id').eq(1)
...
describe('DELETE /api/tasks/:id', () => {
    it('should DELETE an existing task', (done) => {
        const taskId = 1
        chai.request(server).delete('/api/tasks/' + taskId).end((err, response) => {
            response.should.have.status(200)
            done()
        })
    })
